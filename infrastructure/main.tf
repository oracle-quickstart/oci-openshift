...
resource "oci_load_balancer_backend" "openshift_cluster_api_backend_set_external_backends" {
  ...
  port              = 6443
  ip_address        = !local.is_control_plane_iscsi_type && !local.is_compute_iscsi_type ? data.oci_core_vnic.control_plane_primary_vnic[count.index].private_ip_address : data.oci_core_vnic.control_plane_secondary_vnic[count.index].private_ip_address
}
...
resource "oci_load_balancer_backend" "openshift_cluster_ingress_https_backend_set_backends" {
  ...
  port              = 443
  ip_address        = !local.is_compute_iscsi_type ? data.oci_core_vnic.compute_primary_vnic[count.index].private_ip_address : data.oci_core_vnic.compute_secondary_vnic[count.index].private_ip_address
}
...
resource "oci_core_vnic_attachment" "control_plane_secondary_vnic_attachment" {
  ...
  create_vnic_details {
    ...
    display_name = "vnic2"
    hostname_label = oci_core_instance.control_plane_node[count.index].display_name
...
resource "oci_core_vnic_attachment" "compute_secondary_vnic_attachment" {
  ...
  create_vnic_details {
    ...
    display_name = "vnic2"
    hostname_label = oci_core_instance.compute_node[count.index].display_name
...
resource "oci_core_instance" "compute_node" {
  ...
  compartment_id      = var.compartment_ocid
  availability_domain = data.oci_identity_availability_domain.availability_domain.name
  display_name        = "${var.cluster_name}-compute-${count.index + 1}"
